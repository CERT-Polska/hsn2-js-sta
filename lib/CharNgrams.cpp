...
void CharNgrams::addTokens()
{
    ...
    int i=0;
	while (( !inFileName.isEmpty() && ( c[0] = (char) toupper(fgetc( fp ) )) != EOF ) ||
		(!buf.isEmpty() && i<buf.getSize()-1) )
	{
		if (inFileName.isEmpty()) {		
			c[0]=toupper(buf.substring(i,1).c_str()[0]);
		}
		i++;
		if ( isStopChar( c[0] ) )
		{
...
string CharNgrams::output(int limit)
{
        ...
        //for ( int i=1; i<=ngramN; i++ )
        {
                ...
                for ( unsigned j=0; j < count; j++ )
                {
						...
						if (j < limit) {
							out = out + ngramToken->ngram + " ";// +"\t" + ngramToken->value.frequency + "\n";
						}
						delete ngramToken;
                }
        }
